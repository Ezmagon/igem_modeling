...
%modelFileName = 'yeastGEM.mat';
%modelDirectory = getDistributedModelFolder(modelFileName); %Look up the folder for the distributed Models.
%modelFileName= [modelDirectory filesep modelFileName]; % Get the full path. Necessary to be sure, that the right model is loaded
%model = readCbModel(modelFileName);
fprintf('loading model done');
%% add TCA and styrene as metobalites
model = addMetabolite(model,'TCA[c]','trans-cinnamic acid','C9H8O2');
model = addMetabolite(model,'Styr[c]','Styrene','C8H8');
%add reactions and gene associations
model = addReaction(model, 'TCA_prod',...
'metaboliteList', {'s_0794[c]' , 's_0419[c]' , 's_1543[c]','TCA[c]' },...
'stoichCoeffList', [-1; -1; 1; 1;], 'reversible', false);

model = addReaction(model, 'Styr_prod',...
'metaboliteList', {'TCA[c]' , 'Styr[c]' , 's_0456[c]' },...
'stoichCoeffList', [-1; 1; 1;], 'reversible', false);

model = addReaction(model, 'Styr_sink', 'metaboliteList', {'Styr[c]'} ,'stoichCoeffList', -1);

model = changeGeneAssociation(model, 'TCA_prod', 'PAL2');
model = changeGeneAssociation(model, 'Styr_prod', 'FDC1');

%G_YDR539W: FDC1
%s_0456[c]: co2
%s_1032[c]: phenylalanine
%s_0419[c]: amonium
%s_0794[c]: H
...
fprintf('The maximum growth rate is %1.2f \n', growthRate.f);

model = changeObjective(model, 'Styr_sink');
maxSucc = optimizeCbModel(model);
fprintf('The maximum production rate of Styrene is %1.2f \n', maxSucc.f);
%% 
...
%constrain growth for wildtype near max growth
constrWT = struct('rxnList', {{'r_4041'}}, 'rxnValues', 7.90, 'rxnBoundType', 'b');
%constrain styrene and growth to 10% of max
constrMT = struct('rxnList', {{'Styr_sink'}}, 'rxnValues', 497, 'rxnBoundType', 'b');
%constrMT = struct('rxnList', {{'Styr_sink'}}, 'rxnValues',49.7,'rxnBoundType', 'l');
...
constrOpt = struct('rxnList', {{'r_4041', 'Styr_sink'}}, 'values', [0, 497]');
%% 
...
excludedRxns = unique([mustUSet; mustLSet;]);
%% 
...
%excludedRxns = struct('rxnList', {{'Styr_prod'}}, 'typeReg','U');
[optForceSets, posOptForceSets, typeRegOptForceSets, flux_optForceSets] = ...
    optForce(model, targetRxn, biomassRxn, mustU, mustL, ...
             minFluxesW, maxFluxesW, minFluxesM, maxFluxesM, ...
             'k', k, 'nSets', nSets, 'constrOpt', constrOpt, ...
